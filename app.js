...
function drawFrame(ctx, scene, width, height, options = {}) {
  ...
  const hasVoiceover = Boolean(!thumbMode && scene?.voiceover?.trim());
  ...
  const panelRadius = Math.min(width, height) * 0.03;

  ctx.fillStyle = 'rgba(255,255,255,0.08)';
  roundedRect(ctx, leftX, leftY, leftW, leftH, panelRadius);
  ctx.fill();

  ctx.fillStyle = 'rgba(255,255,255,0.08)';
  roundedRect(ctx, rightX, rightY, rightW, rightH, panelRadius);
  ctx.fill();
  ...
  const captionSize = thumbMode ? Math.max(8, width * 0.018) : Math.max(16, width * 0.015);

  const textX = leftX + leftW * 0.07;
  const titleY = leftY + leftH * 0.11;
  const maxTextW = leftW * 0.86;
  const titleLineHeight = titleSize * 1.18;
  const bodyLineHeight = bodySize * 1.32;
  const smallLineHeight = smallSize * 1.3;

  const rightInnerPadX = rightW * 0.04;
  const rightInnerPadY = rightH * 0.04;
  const captionGap = hasVoiceover ? Math.max(12, rightH * 0.022) : 0;
  const captionReservedH = hasVoiceover ? clamp(rightH * 0.24, captionSize * 4.4, rightH * 0.3) : 0;

  const imageX = rightX + rightInnerPadX;
  const imageY = rightY + rightInnerPadY;
  const imageW = rightW - rightInnerPadX * 2;
  const imageH = rightH - rightInnerPadY * 2 - captionReservedH - captionGap;

  if (imageH > 20 && screen?.dataUrl) {
    ...
    if (img.complete && img.naturalWidth > 0) {
      drawContainImage(ctx, img, imageX, imageY, imageW, imageH);
    }
  }
  ...
  let contentTop = titleY + titleMetrics.height + bodySize * 0.7;
  let contentBottom = leftY + leftH - leftH * 0.08;

  if (!thumbMode && scene?.visual) {
    ctx.fillStyle = 'rgba(255,255,255,0.55)';
    ctx.font = `400 ${smallSize}px Inter, Arial, sans-serif`;
    const visualMetrics = measureWrappedText(
      ctx,
      scene.visual,
      maxTextW,
      smallLineHeight,
    ...
    const visualY = leftY + leftH - leftH * 0.08 - visualMetrics.height;
    drawWrappedText(ctx, scene.visual, textX, visualY, maxTextW, smallLineHeight, 2);
    contentBottom = Math.min(contentBottom, visualY - bodySize * 0.8);
  }

  const bullets = Array.isArray(scene?.bullets) ? scene.bullets.filter(Boolean) : [];
  if (bullets.length && contentBottom > contentTop) {
    ctx.fillStyle = 'rgba(255,255,255,0.92)';
    ctx.font = `500 ${bodySize}px Inter, Arial, sans-serif`;

    let cursorY = contentTop;
    const maxBulletLinesPerItem = thumbMode ? 1 : 2;

    bullets.slice(0, 6).forEach((bullet) => {
      const bulletMetrics = measureWrappedText(
        ctx,
        `• ${bullet}`,
        maxTextW,
        bodyLineHeight,
        maxBulletLinesPerItem
      );
      const blockHeight = bulletMetrics.height + bodySize * 0.35;
      if (cursorY + blockHeight > contentBottom) return;
      drawWrappedText(
        ctx,
        `• ${bullet}`,
        textX,
  ...
  if (hasVoiceover) {
    drawVoiceoverOverlay(
      ctx,
      scene.voiceover,
      imageX,
      imageY + Math.max(0, imageH) + captionGap,
      imageW,
      captionReservedH,
      captionSize
    );
...
function drawVoiceoverOverlay(ctx, text, boxX, boxY, boxW, boxH, fontSize) {
  if (!text || boxW <= 0 || boxH <= 0) return;

  const lineHeight = fontSize * 1.35;
  const sidePad = fontSize * 0.95;
  const boxPadY = fontSize * 0.78;
  const textWidth = Math.max(40, boxW - sidePad * 2);

  ctx.save();
  ctx.font = `600 ${fontSize}px Inter, Arial, sans-serif`;
  const maxLines = Math.max(2, Math.floor((boxH - boxPadY * 2) / lineHeight));
  const metrics = measureWrappedText(ctx, text, textWidth, lineHeight, maxLines);
  const actualBoxH = Math.min(boxH, Math.max(metrics.height + boxPadY * 2, fontSize * 2.4));
  const actualBoxY = boxY + Math.max(0, (boxH - actualBoxH) / 2);

  ctx.fillStyle = 'rgba(7, 16, 29, 0.82)';
  roundedRect(ctx, boxX, actualBoxY, boxW, actualBoxH, fontSize * 0.95);
  ctx.fill();
  ...
  drawWrappedText(
    ...
    text,
    boxX + sidePad,
    actualBoxY + boxPadY + fontSize * 0.05,
    textWidth,
    lineHeight,
    maxLines
  );
...
function measureWrappedText(ctx, text, maxWidth, lineHeight, maxLines = 999) {
...
function wrapTextLines(ctx, text, maxWidth, maxLines = 999) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    if (testWidth > maxWidth && line) {
      ...
      if (lines.length >= maxLines - 1) {
        ...
        lines.push(clampTextToWidth(ctx, rest, maxWidth));
        return lines;
      }
    } else {
      line = testLine;
